...
import { updateNewsAction } from '@/app/lib/actions';
...
export default function EditNewsForm({ news, departaments }: any) {
  ...
  const updateNewsWithId = updateNewsAction.bind(null, news._id);
              ...
              <div className="relative mt-2 rounded-md">
                <div className="relative">
                  <input
                    ...
                    name="createdAt"
                    type="date"
                    defaultValue={news.createdAt.split("T")[0]}
                    className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                    aria-describedby="createdAt-error"
                  />
                  <BookmarkIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-gray-500 peer-focus:text-gray-900" />
                </div>
              </div>
...
function UpdateInvoiceButton() {
  ...
  return (
    <Button type="submit" aria-disabled={pending}>Salvar Notícia</Button>
  );
}

...
export default function EditCourseForm({ course }: any) {
  const initialState = { message: null, errors: {} };
  const updateCourseWithId = updateCourseAction.bind(null, course._id);
      ...
      Placeholder.configure({
        placeholder: ({ node }) => {
          if (node.type.name === 'heading' && node.attrs?.level === 1) {
            return "What’s the course's name?";
          }
  ...
  return (
    <form action={dispatch}>
      ...
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        {/* Duration, playlistId e youtubeApiKey */}
        <div className="mb-4">
          <div className="flex gap-2">
            ...
            <div className="flex-1">
              <label htmlFor="playlistId" className="mb-2 block text-sm font-medium">
                playlistId
              </label>
              <div className="relative mt-2 rounded-md">
                <div className="relative">
                  <input
                    ...
                    name="playlistId"
                    type="text"
                    defaultValue={course.playlistId}
                    placeholder="Digite a playlistId"
                    className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                    aria-describedby="playlistId-error"
                  />
                  <BookmarkIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-gray-500 peer-focus:text-gray-900" />
                </div>
              </div>
              {state.errors?.playlistId ? (
                <div
                  id="playlistId-error"
                  aria-live="polite"
                  className="mt-2 text-sm text-red-500"
                >
                  {state.errors.playlistId.map((error: string) => (
                    <p key={error}>{error}</p>
                  ))}
                </div>
              ) : null}
            ...
            <div className="flex-1">
              <label htmlFor="youtubeApiKey" className="mb-2 block text-sm font-medium">
                youtubeApiKey
              </label>
              <div className="relative mt-2 rounded-md">
                <div className="relative">
                  <input
                    ...
                    name="youtubeApiKey"
                    type="text"
                    defaultValue={course.youtubeApiKey}
                    placeholder="Digite a youtubeApiKey"
                    className="peer block w-full rounded-md border border-gray-200 py-2 pl-10 text-sm outline-2 placeholder:text-gray-500"
                    aria-describedby="youtubeApiKey-error"
                  />
                  <BookmarkIcon className="pointer-events-none absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-gray-500 peer-focus:text-gray-900" />
                </div>
              </div>
              {state.errors?.youtubeApiKey ? (
                <div
                  id="youtubeApiKey-error"
                  aria-live="polite"
                  className="mt-2 text-sm text-red-500"
                >
                  {state.errors.youtubeApiKey.map((error: string) => (
                    <p key={error}>{error}</p>
                  ))}
                </div>
              ) : null}
      ...
      <div className="mt-6 flex justify-end gap-4">
        <Link
          href="/dashboard/dynamics-pages/courses"
...
function CreateNewsButton() {
  ...
  return (
    <Button type="submit" aria-disabled={pending}>Salvar Curso</Button>
  );
}
